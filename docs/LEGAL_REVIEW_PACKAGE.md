...
- [ ] הוחלף באנר הטיוטה — הגדר `NEXT_PUBLIC_LEGAL_APPROVED=true` ב-Vercel (לא ממציאים נוסח חדש)
- [ ] תאריך אישור משפטי תועד למטה
- [ ] נסקרו גם `/playbooks/*` (מדריכי חירום — מידע כללי)

**תאריך אישור משפטי:** _טרם_
**שם מאשר:** _טרם_

> **שער קוד:** אין להגדיר `NEXT_PUBLIC_LEGAL_APPROVED=true` עד שכל תיבות ה-checklist מסומנות ע״י עו״ד/DPO אנושי. סוכן CI לא מאשר טקסטים משפטיים.
