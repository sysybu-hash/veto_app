...
> **עדכון:** תיקיית `2026/` הוסרה משורש ה-repo (ניקוי). המסמך נשמר להיסטוריה; למוקאפים קיימים ראו `design_mockups/` ולווב — `web-client/`.

מקור אמת למוקאפים (בעבר): `2026/00_index.html` (27 כרטיסים). עמודת **סטטוס** מתעדכנת לפי ספרינטים; סימון נוכחי משקף את מצב הקוד אחרי סגירת פערי Citizen + AI/Live בתוכנית Gemini Live + Parity.
