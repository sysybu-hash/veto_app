...
export function Skeleton({
  ...
  return (
    <div
      aria-hidden
      className={`animate-pulse bg-surface-sunken ${r} ${className}`}
      style={{
        width,
        height,
        ...style,
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`} aria-hidden>
      {Array.from({ length: lines }).map((_, i) => (
        <div
          key={i}
          className="h-3 animate-pulse rounded-md bg-surface-sunken"
...
export function SkeletonCard({ className = "" }: { className?: string }) {
  return (
    <div
      className={`rounded-2xl border border-slate-200/80 bg-surface-raised p-4 backdrop-blur-xl ${className}`}
      aria-hidden
    >
      <div className="mb-3 h-4 w-1/2 animate-pulse rounded-md bg-surface-sunken" />
