...
class PushService {
  ...
  Future<void> registerLawyerPush() async {
    ...
    try {
      ...
      debugPrint('[PushService] ✅ lawyer push registered');
    } catch (e) {
      debugPrint('[PushService] registration failed (non-fatal): $e');
    }
  }

  /// Web Push for client users (VAPID) — POST /api/users/push-subscription
  Future<void> registerUserPush() async {
    ...
    try {
      ...
      final jsonObj = window['JSON'];
      final jsonStr = jsonObj.callMethod<JSString>('stringify'.toJS, result).toDart;
      final Map<String, dynamic> subscription;
      try {
        subscription =
            Map<String, dynamic>.from(jsonDecode(jsonStr) as Map<dynamic, dynamic>);
      ...
      await http.post(
        Uri.parse('${AppConfig.baseUrl}/users/push-subscription'),
        headers: AppConfig.httpHeaders({'Authorization': 'Bearer $token'}),
        body: jsonEncode({'subscription': subscription}),
      );
      debugPrint('[PushService] ✅ user web push registered');
    } catch (e) {
      debugPrint('[PushService] user registration failed (non-fatal): $e');
    }
  }
}
