...
class _VetoScreenState extends State<VetoScreen> {
    ...
    Widget hubOrWizard(bool wide) {
      if (!_citizenWizardMode) {
        return CitizenHomeHub(
          ...
          inlineAiPanel: _buildInlineAiAssistantCard(isRtl),
        );
      }
  ...
  // CHAT TAB
  // ══════════════════════════════════════════════════════════
  Widget _buildInlineAiAssistantCard(bool isRtl) {
    final recent = _messages.where((m) => !m.isSystem).toList();
    final shown = recent.length > 4 ? recent.sublist(recent.length - 4) : recent;
    final title = _langKey == 'he'
        ? 'סוכן AI מובנה'
        : _langKey == 'ru'
            ? 'Встроенный AI-агент'
            : 'Embedded AI agent';
    final subtitle = _langKey == 'he'
        ? 'צ׳אט טקסט + GEMINI LIVE אודיו, בלי לעבור עמוד'
        : _langKey == 'ru'
            ? 'Текстовый чат + GEMINI LIVE аудио, без перехода'
            : 'Text chat + GEMINI LIVE audio, no page switch';
    final emptyLabel = _langKey == 'he'
        ? 'שאל/י אותי מה לעשות עכשיו, ואני אנחה אותך שלב אחר שלב.'
        : _langKey == 'ru'
            ? 'Спросите, что делать сейчас, и я проведу вас по шагам.'
            : 'Ask what to do now and I will guide you step by step.';
    final textTab = _langKey == 'he'
        ? 'Text'
        : _langKey == 'ru'
            ? 'Текст'
            : 'Text';
    final liveTab = _langKey == 'he'
        ? 'Live Audio'
        : _langKey == 'ru'
            ? 'Live Audio'
            : 'Live Audio';
    final liveHint = _langKey == 'he'
        ? 'מצב דיבור חי עם Gemini. הקש/י על המיקרופון להתחלה/עצירה.'
        : _langKey == 'ru'
            ? 'Живой аудио-режим Gemini. Нажмите микрофон для старта/остановки.'
            : 'Gemini live audio mode. Tap the mic to start/stop.';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(14),
        border: Border.all(color: V26.hairline.withValues(alpha: 0.85)),
        boxShadow: [
          BoxShadow(
            color: V26.ink900.withValues(alpha: 0.04),
            blurRadius: 14,
            offset: const Offset(0, 4),
          ),
        ],
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.auto_awesome_rounded, color: V26.navy600, size: 18),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  title,
          ...
          Row(
            children: [
              Expanded(
                child: _inlineAiModeChip(
                  label: textTab,
                  active: !_inlineAiLiveMode,
                  onTap: () => setState(() => _inlineAiLiveMode = false),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: _inlineAiModeChip(
                  label: liveTab,
                  active: _inlineAiLiveMode,
                  onTap: () => setState(() => _inlineAiLiveMode = true),
                ),
              ),
            ],
          ),
          if (kIsWeb && _liveSessionActive) ...[
            const SizedBox(height: 8),
            _geminiLiveActiveBanner(isRtl),
          ],
          const SizedBox(height: 10),
          if (shown.isEmpty)
            Container(
              ...
              child: Text(
                emptyLabel,
                style: const TextStyle(color: V26.ink500, fontSize: 12.5, height: 1.4),
              ),
            )
          else
            ...shown.map((msg) {
              ...
              return Align(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: isUser
                        ? LinearGradient(
                            ...
                            colors: [
                              V26.navy600.withValues(alpha: 0.16),
                              V26.navy500.withValues(alpha: 0.1),
                            ],
                          )
                  ...
                  child: Text(
                    msg.text,
                    style: TextStyle(
                      color: isUser ? V26.navy600 : V26.ink700,
                      fontSize: 12.5,
                      height: 1.35,
          ...
          if (_isLoading)
            Padding(
              ...
              child: GeminiAiTypingBubble(
                label: '...',
                alignment: isRtl ? Alignment.centerLeft : Alignment.centerRight,
              ),
            ),
          const SizedBox(height: 8),
          if (_inlineAiLiveMode)
            Container(
              padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(
                color: V26.surface.withValues(alpha: 0.75),
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: V26.hairline),
              ),
              child: Row(
                  ...
                  GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: _isListening
                              ? const Color(0xFFFF3B3B).withValues(alpha: 0.72)
                              : V26.hairline,
                        ),
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintStyle:
                          const TextStyle(color: V26.ink500, fontSize: 12.5),
                      contentPadding: const EdgeInsets.symmetric(
                          horizontal: 12, vertical: 10),
                      filled: true,
                      fillColor: V26.surface.withValues(alpha: 0.85),
                      ...
                      focusedBorder: OutlineInputBorder(
                        ...
                        borderSide: BorderSide(
                          color: V26.navy600.withValues(alpha: 0.85),
                          width: 1.2,
                        ),
                      ),
                    ),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: (_isLoading || _isDispatching) ? V26.surface : null,
                      border: Border.all(
                    ...
                    child: Icon(
                      Icons.send_rounded,
                      color:
                          (_isLoading || _isDispatching) ? V26.ink500 : Colors.white,
                      size: 20,
                    ),
                  ),
                ),
        ...
        decoration: BoxDecoration(
          ...
          gradient: active
              ? const LinearGradient(
                  ...
                  colors: [Color(0xFFF6FBFF), Color(0xFFEAF5FF)],
                )
              : null,
          color: active ? null : V26.surface.withValues(alpha: 0.75),
          border: Border.all(
            color: active
          ...
          boxShadow: active
              ? [
                  BoxShadow(
                    color: V26.navy500.withValues(alpha: 0.16),
                    blurRadius: 14,
                    offset: const Offset(0, 3),
                  ),
                ]
