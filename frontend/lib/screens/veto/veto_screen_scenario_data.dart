// Scenario copy: legal content by locale (split from veto_screen_models).
...
_Scenario? _scenarioFromAssetKey(String k) {
  switch (k) {
    case 'policeInquiry':
      return _Scenario.policeInquiry;
    case 'trafficStop':
      return _Scenario.trafficStop;
    case 'civilDispute':
      return _Scenario.civilDispute;
    case 'labor':
      return _Scenario.labor;
    case 'family':
      return _Scenario.family;
    case 'consumer':
      return _Scenario.consumer;
    default:
      return null;
  }
}

_SD _sdWithTitleOverrides(
  _SD base, {
  String? he,
  String? ru,
  String? en,
  ...
  return _SD(
    ...
    ru: ru ?? base.ru,
    en: en ?? base.en,
    tileDHe: base.tileDHe,
    tileDRu: base.tileDRu,
    tileDEn: base.tileDEn,
    headSubHe: base.headSubHe,
    headSubRu: base.headSubRu,
    headSubEn: base.headSubEn,
    knowHe: base.knowHe,
    knowRu: base.knowRu,
    knowEn: base.knowEn,
    firstHe: base.firstHe,
    firstRu: base.firstRu,
    firstEn: base.firstEn,
    warnHe: base.warnHe,
    warnRu: base.warnRu,
    warnEn: base.warnEn,
    rHe: base.rHe,
...
/// Loads [assets/l10n/scenarios_bundle.json] and merges optional title overrides (`he` / `ru` / `en`)
/// onto the built-in [_sdMap]. Returns null if the asset is missing, invalid, or `overrides` is empty.
Future<Map<_Scenario, _SD>?> _mergeScenarioBundleFromAssets(
  Map<_Scenario, _SD> base,
) async {
  try {
    final raw =
        await rootBundle.loadString('assets/l10n/scenarios_bundle.json');
    final decoded = jsonDecode(raw) as Map<String, dynamic>;
    final overrides = decoded['overrides'];
    if (overrides is! Map<String, dynamic> || overrides.isEmpty) {
      return null;
    }
    final out = Map<_Scenario, _SD>.from(base);
    var touched = false;
    for (final e in overrides.entries) {
      final scenario = _scenarioFromAssetKey(e.key);
      if (scenario == null) continue;
      final o = e.value;
      if (o is! Map<String, dynamic>) continue;
      final he = o['he'] as String?;
      final ru = o['ru'] as String?;
      final en = o['en'] as String?;
      if (he == null && ru == null && en == null) continue;
      out[scenario] = _sdWithTitleOverrides(
        out[scenario]!,
        he: he,
        ru: ru,
        en: en,
