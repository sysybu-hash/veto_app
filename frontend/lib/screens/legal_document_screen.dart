// ============================================================
//  LegalDocumentScreen — VETO 2026
//  Pixel-aligned with design_mockups/2026/legal-tools.html (document section).
//
//  "Print page" feel: 780px max width, 60px padding, serif body 16/1.85,
//  centred title, meta line, hairline divider.
//  Top: tabs Privacy / Terms.
// ============================================================
...
class _LegalDocumentScreenState extends State<LegalDocumentScreen> {
  late LegalDocKind _kind = widget.kind;

  static const _titles = <String, Map<LegalDocKind, String>>{
    'he': {LegalDocKind.privacy: 'מדיניות פרטיות', LegalDocKind.terms: 'תנאי שימוש'},
    'en': {LegalDocKind.privacy: 'Privacy Policy', LegalDocKind.terms: 'Terms of Service'},
    'ru': {LegalDocKind.privacy: 'Политика конфиденциальности', LegalDocKind.terms: 'Условия использования'},
  };

  static const _meta = <String, String>{
    'he': 'VETO LEGAL · עודכן 30 באפריל 2026',
    'en': 'VETO LEGAL · Updated April 30, 2026',
    'ru': 'VETO LEGAL · Обновлено 30 апреля 2026',
  };

  static const _bodies = <String, Map<LegalDocKind, String>>{
    'he': {LegalDocKind.privacy: _kPrivacyHe, LegalDocKind.terms: _kTermsHe},
    'en': {LegalDocKind.privacy: _kPrivacyEn, LegalDocKind.terms: _kTermsEn},
    'ru': {LegalDocKind.privacy: _kPrivacyRu, LegalDocKind.terms: _kTermsRu},
  };
  ...
  Widget build(BuildContext context) {
    ...
    final title = _titles[lang]?[_kind] ?? _titles['en']![_kind]!;
    final body = _bodies[lang]?[_kind] ?? _bodies['en']![_kind]!;
    final meta = _meta[lang] ?? _meta['en']!;
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: _DocTabs(active: _kind, onChanged: (k) => setState(() => _kind = k), code: lang),
        ),
        body: SafeArea(
          child: Center(
              ...
              child: SingleChildScrollView(
                ...
                child: Container(
                  ...
                  padding: EdgeInsets.symmetric(
                    horizontal: MediaQuery.of(context).size.width < 640 ? 24 : 60,
                    vertical: MediaQuery.of(context).size.width < 640 ? 32 : 60,
                  ),
                  child: Column(
                    ...
                    children: [
                      Text(
                        ...
                        style: VetoTokens.serif(32, FontWeight.w700, color: VetoTokens.ink900, letterSpacing: -0.3),
                      ),
                      ...
                      const Divider(color: VetoTokens.hairline, thickness: 1, height: 1),
                      const SizedBox(height: 18),
                      SelectableText(
                        body,
                        style: VetoTokens.serif(15, FontWeight.w500, color: VetoTokens.ink800, height: 1.85),
                      ),
                    ],
                  ),
                ),
...
class _DocTabs extends StatelessWidget {
  const _DocTabs({required this.active, required this.onChanged, required this.code});
  final LegalDocKind active;
  final ValueChanged<LegalDocKind> onChanged;
  final String code;

  String _label(LegalDocKind k) {
    switch (code) {
      case 'en': return k == LegalDocKind.privacy ? 'Privacy' : 'Terms';
      case 'ru': return k == LegalDocKind.privacy ? 'Конфиденциальность' : 'Условия';
      default:   return k == LegalDocKind.privacy ? 'פרטיות' : 'תנאי שימוש';
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          for (final k in LegalDocKind.values) _tab(k),
        ],
      ),
    );
  }

  Widget _tab(LegalDocKind k) {
    final selected = k == active;
    return Material(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          child: Text(
            _label(k),
            style: VetoTokens.labelMd.copyWith(color: selected ? VetoTokens.navy700 : VetoTokens.ink500),
          ),
        ),
      ),
    );
...
// --- Template text — preserved verbatim from legacy ----------------
...
7. שינויים לתנאים
ייתכנו עדכונים. המשך שימוש אחרי פרסום שינוי מהווה, ככל שהדין מאפשר, הסכמה.
