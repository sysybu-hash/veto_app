...
class _Sub {
  ...
  factory _Sub.fromJson(Map<String, dynamic> j) {
    ...
    return _Sub(
      id: j['_id'] ?? j['id'] ?? '',
      userId:
          isUserFormat ? (j['_id'] ?? '') : (user['_id'] ?? j['userId'] ?? ''),
      userEmail: isUserFormat
          ? (j['email'] ?? '')
          : (user['email'] ?? j['email'] ?? ''),
      userName: isUserFormat
          ? (j['full_name'] ?? '')
          : (user['name'] ?? user['full_name'] ?? j['userName'] ?? ''),
      phone: j['phone'] ?? user['phone'] ?? '',
      plan: isUserFormat
          ? (j['manually_added'] == true
              ? 'free'
              : (j['is_subscribed'] == true ? 'pro' : 'none'))
          : (j['plan'] ?? 'free'),
      status: isUserFormat
          ? (j['computed_status'] ?? 'no_subscription')
          : (j['status'] ?? 'active'),
      amount: ((j['amount'] ?? j['price'] ?? 0) as num).toDouble(),
      startDate: DateTime.tryParse(j['startDate'] ?? j['createdAt'] ?? ''),
      endDate: DateTime.tryParse(
          j['endDate'] ?? j['subscription_expiry'] ?? j['expiresAt'] ?? ''),
  ...
  String statusLabel(String code) {
    switch (status) {
      case 'active':
        return _t(code, 'statusActive');
      case 'free':
        return _t(code, 'statusFree');
      case 'trial':
        return _t(code, 'statusTrial');
      case 'expired':
        return _t(code, 'statusExpired');
      case 'cancelled':
        return _t(code, 'statusCancelled');
      case 'no_subscription':
        return _t(code, 'statusNoSub');
      case 'unverified':
        return _t(code, 'statusUnverified');
      default:
        return status;
    }
  }
  ...
  String planLabel(String code) {
    switch (plan) {
      case 'free':
        return _t(code, 'planFree');
      case 'basic':
        return _t(code, 'planBasic');
      case 'pro':
        return _t(code, 'planPro');
      case 'none':
        return _t(code, 'planNone');
      default:
        return plan;
    }
  }
...
class _LoginLog {
  ...
  String eventLabel(String code) {
    switch (event) {
      case 'register':
        return _t(code, 'logRegister');
      case 'otp_request':
        return _t(code, 'logOtpReq');
      case 'otp_success':
        return _t(code, 'logOtpOk');
      case 'otp_fail':
        return _t(code, 'logOtpFail');
      case 'google_login':
        return _t(code, 'logGoogle');
      case 'google_fail':
        return _t(code, 'logGoogleFail');
      default:
        return event;
    }
  }
...
class _SubscriptionAdminScreenState extends State<SubscriptionAdminScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (usersRes.statusCode == 200) {
        ...
        _totalRevenue =
            _subs.where((s) => s.amount > 0).fold(0, (s, x) => s + x.amount);
        _monthlyRevenue = _subs
            .where(
                (s) => s.status == 'active' && s.startDate?.month == now.month)
      ...
      if (logsRes.statusCode == 200) {
        ...
        _logs = (list as List)
            .map((e) => _LoginLog.fromJson(e as Map<String, dynamic>))
            .toList();
      }
    } catch (e) {
  ...
  String _userApiId(_Sub s) => s.userId.isNotEmpty ? s.userId : s.id;
        ...
        builder: (ctx, setDlg) => AlertDialog(
          ...
          title: Text(_t(code, 'edit'),
              style: const TextStyle(
                  color: VetoTokens.ink900, fontWeight: FontWeight.w800)),
              ...
              children: [
                ...
                ListTile(
                  ...
                  trailing: Switch(
                    ...
                    activeTrackColor: VetoTokens.navy600.withValues(alpha: 0.4),
                    activeThumbColor: VetoTokens.navy600,
                  ),
                ),
                ListTile(
                  contentPadding: EdgeInsets.zero,
                ...
                Row(children: [
                  Expanded(
                    child: Text(
                      ...
                      style: const TextStyle(
                          color: VetoTokens.ink500, fontSize: 13),
                    ),
                  ),
                  TextButton(
                    onPressed: () async {
  ...
  Future<bool> _confirm(String msg) async =>
      await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(msg,
              style: const TextStyle(color: VetoTokens.ink900, fontSize: 15)),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(ctx, false),
            ...
            FilledButton(
              ...
              child:
                  Text(_t(context.read<AppLanguageController>().code, 'yes')),
            ),
          ],
        ),
      ) ==
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
                icon:
                    const Icon(Icons.refresh_rounded, color: VetoTokens.ink900),
                onPressed: _load,
          ...
          bottom: TabBar(
            ...
            tabs: [
              Tab(
                  text: _t(code, 'tabUsers'),
                  icon: const Icon(Icons.people_rounded, size: 18)),
              Tab(
                  text: _t(code, 'tabLogs'),
          ...
          child: _loading
              ...
              : _loadError != null
                  ? Center(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(Icons.cloud_off_rounded,
                            size: 48, color: VetoTokens.emerg),
                        const SizedBox(height: 12),
                        Text(_loadError!,
                            style: const TextStyle(
                                color: VetoTokens.emerg, fontSize: 14),
                  ...
                  : TabBarView(
                      ...
                      children: [
                        _buildUsersTab(
                            code, activeCount, freeCount, expiredCount),
                        _buildLogsTab(code),
                      ],
                    ),
        ),
  ...
  Widget _buildUsersTab(
      String code, int activeCount, int freeCount, int expiredCount) {
    return Column(children: [
      ...
      Container(
        ...
        child: Column(children: [
          ...
          Row(children: [
            _CountBadge(
                _t(code, 'total'), _subs.length, const Color(0xFF64748B)),
            const SizedBox(width: 8),
            _CountBadge(_t(code, 'active'), activeCount, VetoTokens.ok),
            const SizedBox(width: 8),
            _CountBadge('Free', freeCount, const Color(0xFF5B8FFF)),
            const SizedBox(width: 8),
            _CountBadge(_t(code, 'expired'), expiredCount, VetoTokens.warn),
          ]),
        ]),
      ),
      // ── Search ──────────────────────────────────────────────
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: const Icon(Icons.search_rounded,
                color: VetoTokens.ink500, size: 20),
            filled: true,
            fillColor: const Color(0xFF0F1A24),
        ...
        child: _filtered.isEmpty
            ? Center(
                child: Text(_t(code, 'noSubs'),
                    style: const TextStyle(
                        color: VetoTokens.ink500, fontSize: 15)))
            : ListView.separated(
...
class _LogCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = log.success ? VetoTokens.ok : VetoTokens.emerg;
    final ts =
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          if (log.errorMsg != null) ...[
            const SizedBox(height: 2),
            Text(log.errorMsg!,
                style: const TextStyle(color: VetoTokens.emerg, fontSize: 11)),
          ],
        ])),
      ]),
    );
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          CircleAvatar(
            radius: 18,
            backgroundColor: VetoTokens.navy600.withValues(alpha: 0.12),
            child: Text(
        ...
        Wrap(spacing: 14, runSpacing: 4, children: [
          ...
          if (sub.amount > 0)
            _InfoPill(Icons.attach_money_rounded,
                '\$${sub.amount.toStringAsFixed(2)}', VetoTokens.ok),
